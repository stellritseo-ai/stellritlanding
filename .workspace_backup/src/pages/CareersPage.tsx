...
export default function CareersPage() {
  useEffect(() => { document.title = "Careers — Join StellR IT LLC"; }, []);
  ...
  return (
    <main className="relative min-h-screen">
      ...
      <PageHero
        eyebrow="Careers"
        title={<>Make the best work of your <em className="font-serif italic text-[#c9a4ff]">career</em>.</>}
      ...
      <section className="relative z-10 px-6 pb-24 md:px-12 lg:px-20">
        <div className="mx-auto grid max-w-[1400px] grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {PERKS.map((p, i) => (
            <div key={p.t} className="border-t border-white/10 pt-7">
      ...
      <section className="relative z-10 px-6 py-24 md:px-12 lg:px-20">
        <div className="mx-auto max-w-[1400px]">
          <h2 className="font-serif text-[44px] leading-[1.05] tracking-tight text-white md:text-[72px]">Open roles.</h2>
          <div className="mt-12 divide-y divide-white/10 border-y border-white/10">
            {ROLES.map((r, i) => (
              <a key={r.title} href={`mailto:careers@stellr.it?subject=Application: ${r.title}`} className="group grid grid-cols-1 items-center gap-3 py-6 md:grid-cols-[1.4fr_1fr_1fr_auto] md:gap-8">
                ...
                <span className="flex items-center gap-2 text-[14px] text-white/70"><MapPin className="h-4 w-4" /> {r.location}</span>
                <span className="flex items-center gap-2 text-[13px] text-white/70">{r.type}<span className="grid h-9 w-9 place-items-center rounded-full border border-white/20 text-white transition group-hover:border-[#ff8a5b] group-hover:text-[#ff8a5b]"><ArrowUpRight className="h-4 w-4" /></span></span>
              </a>
            ))}
          </div>
          <p className="mt-10 text-[14px] text-white/60">Don't see your role? Email <a href="mailto:careers@stellr.it" className="text-white underline underline-offset-4 hover:text-[#ff8a5b]">careers@stellr.it</a> with what you'd bring.</p>
        </div>
      </section>
      <Footer />
      <ChatWidget />
